...
function hasConcreteNumberOrAmount(value, { financial = false } = {}) {
  ...
  return /\b(?:gelombang\s+(?:khusus|sisipan\s*\d+|[ivx]+|\d+)\s*[a-c]?|(?:no\.?\s*)?\d{1,4}\s*\/\s*[a-z0-9.-]+\s*\/\s*[a-z0-9.-]+(?:\s*\/\s*[a-z0-9.-]+)*|\d+[.,]?\d*\s*(?:juta|ribu|sks|semester|tahun|bulan|hari|minggu|orang|kali|lokasi|kampus|cabang|ukm|ormawa|organisasi|unit|himaprodi|hima|himpunan|prodi|program|jurusan|beasiswa|fasilitas|layanan|kata|karakter|halaman|lembar|poin|huruf|angka|%)|\d{1,3}(?:\.\d{3})+|\d+\s*\/\s*\d+)\b/i.test(text);
}
