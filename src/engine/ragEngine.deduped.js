...
const { deriveProgramAlias: deriveProgramAliasFromModule, VALID_PROGRAMS: VALID_PROGRAMS_MODULE } = require('./deriveProgramAlias');
const VALID_PROGRAMS = VALID_PROGRAMS_MODULE;
...
function expandWaveVariants(text) {
  const variants = new Set();
  const normalized = String(text || '').toLowerCase();
  const map = [
    ['1a', 'I A'], ['2b', 'II B'], ['3b', 'III B'], ['4a', 'IV A'],
    ['1', 'I'], ['2', 'II'], ['3', 'III'], ['4', 'IV']
  ];
  for (const [src, target] of map) {
    const reSrc = new RegExp(`\\b${src}\\b`, 'i');
    const reTarget = new RegExp(`\\b${target.replace(/\s+/g, '\\s+')}\\b`, 'i');
    if (reSrc.test(normalized)) variants.add(target);
    if (reTarget.test(normalized)) variants.add(src);
  }
...
function tokenizeForSearch(text) {
  ...
  if (!clean) return [];
  const stop = new Set([
    'yang', 'dan', 'atau', 'di', 'ke', 'dari', 'untuk', 'dengan', 'apa', 'itu', 'ini',
    'kak', 'min', 'dong', 'ya', 'nih', 'nya', 'gimana', 'bagaimana', 'berapa', 'kapan',
    'dimana', 'mana', 'tolong', 'mohon', 'saya', 'kami', 'aku', 'mau', 'ingin', 'bisa',
    'boleh', 'tanya', 'cocok', 'jurusan', 'program', 'studi', 'prodi', 'apa', 'yang', 'itu'
  ]);
  return clean
    .split(' ')
    .map((tok) => tok.trim())
    .filter(Boolean)
    .filter((tok) => tok.length >= 2 || ['si','ti','bd','sk','mi','dkv','trpl','tk','mm','an','dg','rpl','dnui','utb','help'].includes(tok))
    .filter((tok) => !stop.has(tok));
}

function scoreIndexByBm25(index, queries) {
  if (!Array.isArray(index) || !index.length) return [];
  const queryTerms = uniqueItems((Array.isArray(queries) ? queries : []).flatMap((q) => tokenizeForSearch(q)));
  if (!queryTerms.length) return index.map(() => 0);

  const docs = index.map((item) => {
    const text = `${String(item.chunk || '')} ${String(item.filename || item.trainingId || '')}`;
    const tokens = tokenizeForSearch(text);
    const len = tokens.length || 1;
    const tf = new Map();
    for (const token of tokens) tf.set(token, (tf.get(token) || 0) + 1);
    return { tf, len, tokens: Array.from(new Set(tokens)) };
  });

  const docCount = docs.length;
  const avgDl = docs.reduce((sum, doc) => sum + doc.len, 0) / docCount;
  const k1 = 1.2;
  const b = 0.75;
  const idf = new Map();
  ...
  for (const term of queryTerms) {
    ...
    for (const doc of docs) {
      if (doc.tokens.includes(term)) docFreq += 1;
    }
    idf.set(term, Math.log(1 + ((docCount - docFreq + 0.5) / (docFreq + 0.5))));
  }
  ...
  const rawScores = docs.map((doc) => {
    ...
    for (const term of queryTerms) {
      ...
      const denom = termFreq + k1 * (1 - b + b * (doc.len / avgDl));
      score += termIdf * ((termFreq * (k1 + 1)) / denom);
    }
    return score;
  });

  const maxScore = Math.max(...rawScores, 0.000001);
  return rawScores.map((score) => Math.min(1, score / maxScore));
}

function computeHybridRetrievalScore(semanticScore, bm25Score, keywordScore, metadataBoost, exactBoost) {
  const semanticFactor = Number.isFinite(semanticScore) ? semanticScore : 0;
  const bm25Factor = Number.isFinite(bm25Score) ? bm25Score : 0;
  const keywordFactor = Number.isFinite(keywordScore) ? keywordScore : 0;
  const metadataFactor = Number.isFinite(metadataBoost) ? Math.tanh(metadataBoost) : 0;
  const exactFactor = Number.isFinite(exactBoost) ? Math.min(0.2, exactBoost * 0.1) : 0;
...
function contextSimilarity(itemA, itemB) {
  if (!itemA || !itemB) return 0;
  if (Array.isArray(itemA.embedding) && Array.isArray(itemB.embedding) && itemA.embedding.length === itemB.embedding.length) {
    return Math.max(0, cosineSimilarity(itemA.embedding, itemB.embedding));
  }
  const tokensA = new Set(tokenizeForSearch(String(itemA.chunk || '')));
  const tokensB = new Set(tokenizeForSearch(String(itemB.chunk || '')));
  if (!tokensA.size || !tokensB.size) return 0;
  const overlap = Array.from(tokensA).filter((tok) => tokensB.has(tok)).length;
  return overlap / Math.max(tokensA.size, tokensB.size);
}

function selectDiverseTopK(candidates, topK, lambda = 0.65) {
  if (!Array.isArray(candidates) || candidates.length <= topK) return Array.isArray(candidates) ? candidates.slice(0, topK) : [];
  const selected = [];
  ...
  while (selected.length < topK && remaining.length) {
    ...
    for (let i = 0; i < remaining.length; i += 1) {
      ...
      const similarity = Math.max(...selected.map((item) => contextSimilarity(item.item, candidate.item), 0));
      const score = (lambda * (candidate.hybridScore || candidate.compositeScore || 0)) - ((1 - lambda) * similarity);
      if (score > bestScore) {
        bestScore = score;
        bestIndex = i;
      }
...
function buildSearchQueries(rawQuery, queryEntities) {
  const query = String(rawQuery || '').trim();
  if (!query) return [''];
  const base = normalizeQueryForRetrieval(query);
  const variants = [base];
  const lower = base.toLowerCase();

  const programAlias = queryEntities && queryEntities.program ? String(queryEntities.program).toUpperCase().trim() : null;
  const programMap = {
    TI: 'teknologi informasi',
    SI: 'sistem informasi',
    SK: 'sistem komputer',
  ...
  if (programAlias && programMap[programAlias]) {
    variants.push(`${base} ${programMap[programAlias]}`);
    variants.push(`${base} program studi ${programMap[programAlias]}`);
  }

  if (/\b(ukm|ormawa)\b/i.test(lower)) {
    variants.push(`${base} daftar ukm itb stikom bali`);
    variants.push(`${base} ukm ormawa`);
  }

  if (/\b(biaya|dpp|ukt|spp|pendaftaran|gelombang|diskon|potongan)\b/i.test(lower)) {
    variants.push(`${base} program studi biaya pendaftaran`);
  }

  for (const variant of expandWaveVariants(lower)) {
    if (!lower.includes(variant.toLowerCase())) {
      variants.push(`${base} ${variant}`);
    }
  }

  return uniqueItems(variants).slice(0, 4);
}

function getHybridRetrievalQueries(rawQuery, queryEntities) {
  return buildSearchQueries(rawQuery, queryEntities);
}

function normalizeQueryForSearch(rawQuery) {
  return String(buildSearchQueries(rawQuery, null)[0] || rawQuery || '').trim();
}

function computeHybridCandidateScore(item, question, intent, semanticScore, queryEntities, bm25Score) {
  const breakdown = getChunkScoreBreakdown(item, question, intent, semanticScore, queryEntities);
  const keywordScore = Math.max(getChunkKeywordScore(item.chunk, question), bm25Score);
  const hybridScore = computeHybridRetrievalScore(semanticScore, bm25Score, keywordScore, breakdown.metadataBoost, breakdown.exactBoost);
  return {
    ...breakdown,
    score: semanticScore,
    semanticScore,
...
function filterCandidatesByMetadata(candidates, queryEntities, userIntent) {
  if (!Array.isArray(candidates) || !queryEntities) return candidates;
  const forbidden = getForbiddenDocCategories(userIntent || 'GENERAL');
  return candidates.filter((candidate) => {
    if (!candidate || !candidate.item) return false;
    const category = String(candidate.item.docCategory || candidate.item.category || 'UNKNOWN').toUpperCase();
    if (forbidden.has(category)) return false;
    if (queryEntities.program && candidate.item && candidate.item.sectionTitle) {
      const programLabel = String(queryEntities.program || '').toLowerCase();
      return String(candidate.item.sectionTitle || '').toLowerCase().includes(programLabel) || String(candidate.item.chunk || '').toLowerCase().includes(programLabel) || category !== 'UNKNOWN';
    }
    return true;
  });
}

function penalizeDuplicateCandidates(candidates) {
  const seen = new Set();
  return candidates.filter((candidate) => {
    if (!candidate || !candidate.item) return false;
    const hash = chunkHash(candidate.item.chunk);
...
function maybeApplyDiversity(candidates, topK) {
  const enabled = String(process.env.RAG_DIVERSITY_SELECTION || 'true').toLowerCase() !== 'false';
  if (!enabled) return candidates.slice(0, topK);
  const lambda = Number.isFinite(Number(process.env.RAG_MMR_LAMBDA)) ? Number(process.env.RAG_MMR_LAMBDA) : 0.65;
  return selectDiverseTopK(candidates, topK, lambda);
}

function computeRetrievalScores(index, question, intent, queryEntities, searchQueries, qEmb) {
  const bm25Scores = scoreIndexByBm25(index, searchQueries);
  return index.map((item, idx) => {
    const semanticScore = qEmb && Array.isArray(item.embedding) ? cosineSimilarity(qEmb, item.embedding) : 0;
    const candidate = computeHybridCandidateScore(item, question, intent, semanticScore, queryEntities, bm25Scores[idx] || 0);
    return { item, score: semanticScore, semanticScore, bm25Score: bm25Scores[idx] || 0, keywordScore: candidate.keywordScore, finalScore: candidate.finalScore, compositeScore: candidate.compositeScore, attributeScore: candidate.attributeScore, metadataBoost: candidate.metadataBoost, scoreComponents: candidate.scoreComponents, itemEntities: candidate.itemEntities, itemCategory: candidate.itemCategory, queryCategory: candidate.queryCategory, exactMatch: candidate.exactMatch };
  });
}

function normalizeSemanticConfidence(score) {
  if (!Number.isFinite(score)) return 0;
  return Math.max(0, Math.min(1, score));
}

function penalizeDuplicateChunks(candidates) {
  const seen = new Map();
  for (const candidate of candidates) {
    ...
    if (count > 0) {
      candidate.compositeScore -= 0.08 * Math.min(3, count);
      candidate.score -= 0.04 * Math.min(3, count);
    }
...
function getSearchQueriesForRetrieval(queryForRetrieval, queryEntities) {
  return buildSearchQueries(queryForRetrieval, queryEntities);
}

function getRetrievalEmbedding(query, cachedEmbedding = null) {
  return cachedEmbedding || computeEmbedding(query);
}

function buildQuerySearchCandidates(question, queryForRetrieval, queryEntities) {
  return getSearchQueriesForRetrieval(queryForRetrieval || question, queryEntities);
}

function getHybridRetrievalModels(index, searchQueries, qEmb, question, intent, queryEntities) {
  return computeRetrievalScores(index, question, intent, queryEntities, searchQueries, qEmb);
}

function postProcessRetrievalCandidates(candidates, queryEntities, userIntent, topK) {
  let filtered = filterCandidatesByMetadata(candidates, queryEntities, userIntent);
  filtered = penalizeDuplicateCandidates(filtered);
  filtered.sort((a, b) => b.compositeScore - a.compositeScore);
  return maybeApplyDiversity(filtered, topK);
}

function getQueryVariants(rawQuery, queryEntities) {
  return buildSearchQueries(rawQuery, queryEntities);
}

function getBestSearchQuery(queries) {
  return Array.isArray(queries) && queries.length ? queries[0] : '';
}

function mergeHybridAndDenseScores(candidates) {
  return candidates;
}

function buildRetrievalSignal(question, queryForRetrieval, queryEntities) {
  return getSearchQueriesForRetrieval(queryForRetrieval, queryEntities).join(' | ');
}

function getChunkScoreMetadata(item, queryEntities) {
  const entities = getChunkEntities(item);
  if (queryEntities && queryEntities.program && entities.program === queryEntities.program) return 1;
  return 0;
}

function isEligibleSearchCandidate(item, queryEntities) {
  if (!item || !queryEntities) return true;
  const forbidden = getForbiddenDocCategories(String(queryEntities.intent || 'GENERAL').toUpperCase());
  const category = String(item.docCategory || item.category || 'UNKNOWN').toUpperCase();
  if (forbidden.has(category)) return false;
  return true;
}

function queryExpansionHint(question) {
  const q = String(question || '').toLowerCase();
  if (q.includes('gelombang') && /\b(1|2|3|4)(?:\s*[ab])?\b/i.test(q)) {
    return 'gelombang';
  }
  return '';
}
...
function getCandidateText(item) {
  return `${String(item.chunk || '')} ${String(item.filename || item.trainingId || '')}`.trim();
}
...
function isLikelyStrongProgramMatch(item, queryEntities) {
  if (!item || !queryEntities || !queryEntities.program) return false;
  const program = String(queryEntities.program || '').toLowerCase();
  const content = normalizeCandidateText(String(item.chunk || '') + ' ' + String(item.filename || item.trainingId || ''));
  return content.includes(program);
}

function scoreSearchCandidate(item, queryEntities, question) {
  if (!item) return 0;
  const metadata = getChunkEntities(item);
  let score = 0;
  if (queryEntities && queryEntities.program && metadata.program === queryEntities.program) score += 0.2;
  if (queryEntities && queryEntities.wave && metadata.wave === queryEntities.wave) score += 0.1;
  if (getChunkKeywordScore(item.chunk, question) > 0.2) score += 0.1;
  return score;
}
...
function buildExpandedSearchQuery(rawQuery, queryEntities) {
  return getSearchQueriesForRetrieval(rawQuery, queryEntities).join(' | ');
}

function getQueryExpansionLabel(query) {
  return String(query || '').trim().slice(0, 120);
}

function getQueryScoringLabel(item, query) {
  return String(item.filename || item.trainingId || item.id || 'chunk').trim();
}

function getChunkKeywordCandidates(question, queryEntities, index) {
  if (!Array.isArray(index)) return [];
  return index.map((item) => {
    return {
      item,
      score: getChunkKeywordScore(item.chunk, question)
    };
  }).sort((a,b) => b.score - a.score).slice(0, 25);
}

function getSparseSearchCandidates(question, queryEntities, index) {
  return scoreIndexByBm25(index, buildSearchQueries(question, queryEntities)).map((score, idx) => ({ item: index[idx], score }));
}

function ensureValidRetrievalIndex(index) {
  return Array.isArray(index) ? index : [];
}

function sanitizeQueryForRetrievalText(text) {
  return normalizeQueryForSearch(text);
}

function getQueryExpansionCandidateScore(item, queryEntities, question, searchQueries) {
  const bm25 = scoreIndexByBm25([item], searchQueries)[0] || 0;
  const keyword = getChunkKeywordScore(item.chunk, question);
  return bm25 + keyword;
}
...
function shouldUseHybridRetrieval() {
  return String(process.env.RAG_ENABLE_HYBRID_RETRIEVAL || 'true').toLowerCase() !== 'false';
}

function shouldUseQueryExpansion() {
  return String(process.env.RAG_ENABLE_QUERY_EXPANSION || 'true').toLowerCase() !== 'false';
}

function shouldUseDiversitySelection() {
  return String(process.env.RAG_ENABLE_DIVERSITY_SELECTION || 'true').toLowerCase() !== 'false';
}

function getRetrievalDebugLabel() {
  return `hybrid:${String(process.env.RAG_ENABLE_HYBRID_RETRIEVAL || 'true')}`;
}
...
function prepareRetrievalQueries(question, queryEntities) {
  return getSearchQueriesForRetrieval(question, queryEntities);
}

function scoreHybridCandidate(item, qEmb, question, intent, queryEntities, searchQueries, bm25Score) {
  const semanticScore = qEmb && Array.isArray(item.embedding) ? cosineSimilarity(qEmb, item.embedding) : 0;
  const breakdown = getChunkScoreBreakdown(item, question, intent, semanticScore, queryEntities);
  const keyword = Math.max(getChunkKeywordScore(item.chunk, question), bm25Score);
  const hybrid = computeHybridRetrievalScore(semanticScore, bm25Score, keyword, breakdown.metadataBoost, breakdown.exactBoost);
  return { semanticScore, bm25Score, hybridScore: hybrid, keywordScore: keyword, compositeScore: hybrid, score: semanticScore, ...breakdown };
}

function getTopHybridCandidates(index, question, intent, queryEntities, searchQueries, qEmb) {
  const bm25Scores = scoreIndexByBm25(index, searchQueries);
  return index.map((item, idx) => {
    const semanticScore = qEmb && Array.isArray(item.embedding) ? cosineSimilarity(qEmb, item.embedding) : 0;
    ...
    const breakdown = getChunkScoreBreakdown(item, question, intent, semanticScore, queryEntities);
    const keywordScore = Math.max(getChunkKeywordScore(item.chunk, question), bm25Score);
    const compositeScore = computeHybridRetrievalScore(semanticScore, bm25Score, keywordScore, breakdown.metadataBoost, breakdown.exactBoost);
    return {
      item,
      score: semanticScore,
      semanticScore,
...
function normalizeSearchQueries(queries) {
  return uniqueItems(Array.isArray(queries) ? queries : []).map((q) => normalizeIndonesianQuestionText(q));
}

function getSearchQueryText(query) {
  return String(query || '').trim();
}

function getRetrievalQueryLabel(query) {
  return getSearchQueryText(query).slice(0, 80);
}

function getTopQueryVariant(queries) {
  return Array.isArray(queries) && queries.length ? queries[0] : '';
}

function getSparseRetrievalScore(item, queries) {
  return scoreIndexByBm25([item], queries)[0] || 0;
}

function chooseRetrievalQuery(queries) {
  return getTopQueryVariant(queries);
}

function getRawRetrievalCandidate(item) {
  return item;
}

function isHybridRetrievalEnabled() {
  return shouldUseHybridRetrieval();
}

function shouldFilterByMetadata() {
  return String(process.env.RAG_ENABLE_METADATA_FILTERING || 'true').toLowerCase() !== 'false';
}

function buildRetrievalOptions(options) {
  return {
    topK: Number.isFinite(Number(options && options.topK)) ? Number(options.topK) : parseInt(process.env.RAG_TOP_K || '8', 10),
    strict: options && options.strict === true
  };
}

function getCandidateContextText(item) {
  return String(item.chunk || '').slice(0, 400);
}

function buildRagContextSummary(question, top) {
  return buildMultiDocSummary(top, question);
}

function isQueryAnswerableByStructuredRules(question, queryEntities) {
  return Boolean(queryEntities && queryEntities.intent && queryEntities.intent !== 'unknown');
}

function getPotentialRetrievalPrograms(queryEntities) {
  if (!queryEntities) return [];
  const program = queryEntities.program ? String(queryEntities.program).toUpperCase() : null;
  return program ? [program] : [];
}

function getQuerySearchEmbeddings(searchQueries) {
  return Array.isArray(searchQueries) && searchQueries.length ? searchQueries.map((q) => computeEmbedding(q)) : [];
}

function getQueryRetrievalText(rawQuery, queryEntities) {
  return getSearchQueryText(getTopQueryVariant(buildSearchQueries(rawQuery, queryEntities)));
}

function selectTopHybridCandidates(topK, candidates) {
  return maybeApplyDiversity((Array.isArray(candidates) ? candidates : []).sort((a,b) => b.compositeScore - a.compositeScore), topK);
}

function shouldApplyDiversitySelection() {
  return process.env.RAG_DIVERSITY_SELECTION !== 'false';
}

function buildHybridQueryContextLabel(queries) {
  return uniqueItems(Array.isArray(queries) ? queries : []).join(' | ');
}

function scoreQueryVariant(query, question) {
  return getChunkKeywordScore(query, question);
}

function normalizeCandidateQuery(item) {
  return normalizeCandidateText(getCandidateText(item));
}

function getQueryVariantCount(queries) {
  return Array.isArray(queries) ? queries.length : 0;
}

function getCandidateDocumentScore(item) {
  return normalizeSearchQuery(String(item.chunk || '')).length;
}

function toSearchCandidate(item, score) {
  return { item, score };
}

function getSearchCandidateScore(item, query) {
  return scoreCandidateAgainstQuery(item, query);
}

function scoreCandidateAgainstQuery(item, query) {
  return getChunkKeywordScore(item.chunk, query || '');
}

function isCandidateStrong(item, question) {
  return getChunkKeywordScore(item.chunk, question) > 0.25;
}

function getSearchCandidateSimilarity(a, b) {
  return contextSimilarity(a, b);
}

function getSearchCandidateId(item) {
  return item && (item.id || item.trainingId || item.filename || '').toString();
}

function buildRetrievalDiagnostics(question, candidates) {
  if (!Array.isArray(candidates)) return null;
  return candidates.slice(0, 5).map((candidate) => ({ id: candidate.item && getSearchCandidateId(candidate.item), score: candidate.compositeScore, semantic: candidate.semanticScore, bm25: candidate.bm25Score, keyword: candidate.keywordScore }));
}

function getRetrievalTracePayload(question, searchQueries, candidates) {
  return {
    query: question,
    searchQueries,
    candidateCount: Array.isArray(candidates) ? candidates.length : 0,
    topCandidates: buildRetrievalDiagnostics(question, candidates)
  };
}

function maybeTraceRetrieval(question, searchQueries, candidates) {
  if (!process.env.RAG_DEBUG_CHUNK_SCORING) return;
  logger.info(getRetrievalTracePayload(question, searchQueries, candidates), '[RAG] Hybrid retrieval debug');
}

function extractEffectiveSearchQueries(question, queryEntities) {
  return buildSearchQueries(question, queryEntities);
}

function getQueryRetrievalCandidates(index, question, intent, queryEntities, searchQueries, qEmb) {
  const candidates = getTopHybridCandidates(index, question, intent, queryEntities, searchQueries, qEmb);
  if (shouldFilterByMetadata()) {
    return filterCandidatesByMetadata(candidates, queryEntities, intent);
  }
  return candidates;
}

function getHybridSearchCandidates(index, question, intent, queryEntities, searchQueries, qEmb) {
  const candidates = getQueryRetrievalCandidates(index, question, intent, queryEntities, searchQueries, qEmb);
...
function getFinalHybridCandidates(index, question, intent, queryEntities, searchQueries, qEmb, topK) {
  const candidates = getHybridSearchCandidates(index, question, intent, queryEntities, searchQueries, qEmb);
  const unique = penalizeDuplicateCandidates(candidates);
  const filtered = unique.sort((a, b) => b.compositeScore - a.compositeScore);
  return maybeApplyDiversity(filtered, topK);
}

function getTopHybridResults(index, question, intent, queryEntities, searchQueries, qEmb, topK) {
  return getFinalHybridCandidates(index, question, intent, queryEntities, searchQueries, qEmb, topK);
}

function getSearchQueriesForQuestion(question, queryEntities) {
  return buildSearchQueries(question, queryEntities);
}

function getBestScoreOnCandidates(candidates) {
  if (!Array.isArray(candidates) || candidates.length === 0) return 0;
  return candidates[0].compositeScore || 0;
}

function shouldUseStrictRetrieval() {
  return String(process.env.RAG_STRICT_MODE || 'false').toLowerCase() === 'true';
}

function getQueryExpansionEngineLabel() {
  return 'legacy-hybrid';
}

function scoreTokenMatch(item, question) {
  return getChunkKeywordScore(item.chunk, question);
}

function getSearchQueryFingerprint(queries) {
  return Array.isArray(queries) ? queries.join(' | ') : String(queries || '');
}

function buildQuerySearchPayload(question, queries) {
  return { question, queries: Array.isArray(queries) ? queries : [queries] };
}

function getCandidateSourceType(item) {
  return String(item.sourceType || item.docCategory || item.category || 'unknown').toLowerCase();
}

function getCandidateDocumentId(item) {
  return String(item.id || item.trainingId || item.filename || '').trim();
}
...
function isCandidateValid(item) {
  return Boolean(item && item.chunk);
}

function buildSearchQueryCandidate(item, score) {
  return { item, score };
}

function isHybridQueryCandidate(candidate) {
  return Boolean(candidate && candidate.item);
}

function getHybridQueryCount(candidates) {
  return Array.isArray(candidates) ? candidates.length : 0;
}

function getCandidateSnippet(item) {
  return String(item.chunk || '').slice(0, 120);
}

function getHybridRetrievalSummary(question, queryEntities, candidates) {
  return {
    question,
    queryEntities,
    queryCount: getQueryVariantCount(buildSearchQueries(question, queryEntities)),
    topCandidate: candidates && candidates[0] ? getCandidateTitle(candidates[0].item) : null
  };
}

function getRetrievalQualityLabels(queries) {
  return Array.isArray(queries) ? queries.map((q) => q.slice(0, 50)) : [];
}

function getSearchQueryFingerprintLabel(queries) {
  return getSearchQueryFingerprint(buildSearchQueries(queries, null));
}

function getCandidateMetadataScore(item) {
  return getChunkEntities(item).program ? 0.1 : 0;
}

function getFinalContextCandidates(topScored) {
  return Array.isArray(topScored) ? topScored.slice(0) : [];
}

function getSearchQueryFeatures(query) {
  return { text: String(query || '').trim(), length: String(query || '').trim().split(/\s+/).length };
}

function getRetrievalHyperparams() {
  return {
    bm25Enabled: shouldUseHybridRetrieval(),
    diversityEnabled: shouldUseDiversitySelection(),
    mmrLambda: Number.isFinite(Number(process.env.RAG_MMR_LAMBDA)) ? Number(process.env.RAG_MMR_LAMBDA) : 0.65
  };
}

function getSearchCandidateFeatures(item, question, queryEntities) {
  return {
    programMatch: isLikelyStrongProgramMatch(item, queryEntities),
    keywordMatch: getChunkKeywordScore(item.chunk, question)
  };
}

function getCandidateScoreDetails(item, question, intent, queryEntities, bm25Score) {
  const semanticScore = 0;
  const breakdown = getChunkScoreBreakdown(item, question, intent, semanticScore, queryEntities);
  return { bm25Score, breakdown };
}

function getSearchCandidateNormalization(text) {
  return normalizeCandidateText(text);
}

function buildSearchQueryHint(question) {
  return queryExpansionHint(question);
}

function getCandidateIdentifier(item) {
  return item && item.id ? String(item.id) : null;
}

function scoreCandidateMetadata(item, queryEntities) {
  return scoreSearchCandidate(item, queryEntities, item.chunk || '');
}

function getRetrievalCandidateText(item) {
  return getCandidateText(item);
}

function getHybridScoreComponents(item, question, intent, queryEntities, bm25Score) {
  const semanticScore = 0;
  const breakdown = getChunkScoreBreakdown(item, question, intent, semanticScore, queryEntities);
  const keywordScore = Math.max(getChunkKeywordScore(item.chunk, question), bm25Score);
  return { semanticScore, bm25Score, keywordScore, metadataBoost: breakdown.metadataBoost, exactBoost: breakdown.exactBoost };
}

function getCandidateKey(item) {
  return `${getCandidateIdentifier(item) || 'no-id'}-${chunkHash(item.chunk)}`;
}

function scoreCandidateEmbedding(item, qEmb) {
  return qEmb && Array.isArray(item.embedding) ? cosineSimilarity(qEmb,item.embedding) : 0;
}

function isQueryTokenRelevant(token) {
  return token && token.length >= 2;
}

function getQueryTermCoverage(question, item) {
  const tokens = tokenizeForSearch(question);
  if (!tokens.length) return 0;
  const hay = normalizeCandidateText(String(item.chunk || '')); 
  let matches = 0;
  for (const token of tokens) if (hay.includes(token)) matches += 1;
  return matches / tokens.length;
}

function getCandidateAggregateScore(item, question, intent, queryEntities, qEmb, bm25Score) {
  const semanticScore = scoreCandidateEmbedding(item, qEmb);
  const keywordScore = Math.max(getChunkKeywordScore(item.chunk, question), bm25Score);
  const breakdown = getChunkScoreBreakdown(item, question, intent, semanticScore, queryEntities);
  const hybrid = computeHybridRetrievalScore(semanticScore, bm25Score, keywordScore, breakdown.metadataBoost, breakdown.exactBoost);
  return { semanticScore, bm25Score, keywordScore, hybridScore: hybrid, compositeScore: hybrid, score: semanticScore, breakdown };
}

function getQueryExpansionCandidates(question, queryEntities, index) {
  return index.map((item) => ({ item, score: getQueryTermCoverage(question, item) }));
}

function buildRetrievalQueryCandidates(question, queryEntities, index) {
  const searchQueries = buildSearchQueries(question, queryEntities);
  return index.map((item, idx) => ({ item, bm25: scoreIndexByBm25([item], searchQueries)[0] || 0, keywords: getChunkKeywordScore(item.chunk, question) }));
}

function getQueryExpansionScore(item, query) {
  return getChunkKeywordScore(item.chunk, query);
}

function getSearchQueryTermSet(query) {
  return new Set(tokenizeForSearch(query));
}

function getQueryEntityProgram(queryEntities) {
  return queryEntities && queryEntities.program ? String(queryEntities.program).toUpperCase() : null;
}

function buildQueryExpansionCandidates(question, queryEntities, index) {
  const queries = buildSearchQueries(question, queryEntities);
  return scoreIndexByBm25(index, queries).map((score, idx) => ({ item: index[idx], score }));
}

function getSearchQueryCandidate(item, question, queryEntities) {
  return { item, score: getChunkKeywordScore(item.chunk, question) };
}

function getQueryExpansionPriority(item, queryEntities) {
  return isLikelyStrongProgramMatch(item, queryEntities) ? 1 : 0;
}

function getCandidateTextPreview(item) {
  return String(item.chunk || '').slice(0, 120).replace(/\s+/g, ' ').trim();
}

function getSearchCandidateDebug(item, question, queryEntities, bm25Score) {
  return {
...
function getHybridQuerySummary(question, queryEntities) {
  return { query: question, variantCount: getQueryVariantCount(buildSearchQueries(question, queryEntities)) };
}

function getCandidateRetrievalScore(item, question, queryEntities, qEmb, bm25Score) {
  const semanticScore = scoreCandidateEmbedding(item, qEmb);
  const keywordScore = Math.max(getChunkKeywordScore(item.chunk, question), bm25Score);
  const breakdown = getChunkScoreBreakdown(item, question, qEmb ? semanticScore : 0, queryEntities);
  return computeHybridRetrievalScore(semanticScore, bm25Score, keywordScore, breakdown.metadataBoost, breakdown.exactBoost);
}

function getQueryVariantHints(question) {
  return buildSearchQueries(question, null);
}

function getCandidateWordOverlap(item, question) {
  return getQueryTermCoverage(question, item);
}

function getTopHybridCandidate(item, question, intent, queryEntities, qEmb, bm25Score) {
  const semanticScore = scoreCandidateEmbedding(item, qEmb);
  const keywordScore = Math.max(getChunkKeywordScore(item.chunk, question), bm25Score);
  const breakdown = getChunkScoreBreakdown(item, question, intent, semanticScore, queryEntities);
  return computeHybridRetrievalScore(semanticScore, bm25Score, keywordScore, breakdown.metadataBoost, breakdown.exactBoost);
...
function getHybridRetrievalCandidateMetadata(item) {
  return getChunkEntities(item);
}

function getCandidateRetrievalLabel(item) {
  return getCandidateTitle(item);
}

function getSearchCandidateQuality(item, question, qEmb, bm25Score) {
  return getCandidateRetrievalScore(item, question, null, qEmb, bm25Score);
}

function getQueryVariantList(question, queryEntities) {
  return buildSearchQueries(question, queryEntities);
}

function normalizeSearchQueryVariant(query) {
  return String(query || '').trim();
}

function getCandidateScoreLabel(candidate) {
  return `sem:${candidate.semanticScore?.toFixed(3)||0} bm25:${candidate.bm25Score?.toFixed(3)||0} hybrid:${candidate.compositeScore?.toFixed(3)||0}`;
}

function getRetrievalExplanation(question, queryEntities, candidates) {
  return {
    query: question,
    variants: getQueryVariantList(question, queryEntities),
    topCandidate: candidates && candidates[0] ? getCandidateTitle(candidates[0].item) : null,
    count: getCandidateCount(candidates)
  };
}

function buildHybridRetrievalDebug(question, queryEntities, candidates) {
  return getRetrievalExplanation(question, queryEntities, candidates);
}

function getSearchQueryTokenCount(query) {
  return tokenizeForSearch(query).length;
}

function getCandidateEmbeddingScore(item, qEmb) {
  return scoreCandidateEmbedding(item, qEmb);
}

function getHybridRetrievalContextLabel(question, queryEntities) {
  return buildQueryExpansionLabel(buildSearchQueries(question, queryEntities).join(' | '));
}

function getCandidateDocumentMatch(item, question) {
  return getChunkKeywordScore(item.chunk, question);
}

function buildRagRetrievalContext(question, queryEntities) {
  return buildSearchQueries(question, queryEntities);
}

function getHybridRetrievalSignal(question, queryEntities) {
  return buildQuerySearchPayload(question, buildSearchQueries(question, queryEntities));
}

function getCandidateScoreVector(item, question, queryEntities, qEmb, bm25Score) {
  return {
    semantic: scoreCandidateEmbedding(item, qEmb),
    bm25: bm25Score,
    keyword: getChunkKeywordScore(item.chunk, question),
    metadata: getChunkEntities(item)
  };
}

function scoreHybridRetrievalCandidate(item, question, intent, queryEntities, qEmb, bm25Score) {
  return getCandidateRetrievalScore(item, question, queryEntities, qEmb, bm25Score);
}

function isCandidateMetadataMatch(item, queryEntities) {
  return isLikelyStrongProgramMatch(item, queryEntities);
}

function getSearchQueryVariants(question, queryEntities) {
  return buildSearchQueries(question, queryEntities);
}

function getQueryVariantScore(query, item) {
  return scoreCandidateAgainstQuery(item, query);
}

function getHybridRetrievalInfo(question, queryEntities, candidates) {
  return buildHybridRetrievalDebug(question, queryEntities, candidates);
}

function getCandidateGuidance(item, question) {
  return isCandidateStrong(item, question) ? 'strong' : 'weak';
}

function buildRetrievalQuerySummary(question, queryEntities) {
  return getHybridRetrievalSummary(question, queryEntities);
}

function getCandidateMatchLabel(item, question) {
  return isLikelyStrongProgramMatch(item, queryEntities) ? 'program-match' : 'generic';
}

function getCandidateProgramHint(item) {
  return getChunkEntities(item).program || null;
}

function getHybridRetrievalSignature(question, queryEntities) {
  return getQueryFingerprint(buildSearchQueries(question, queryEntities));
}

function getQueryFingerprint(queries) {
  return Array.isArray(queries) ? queries.join(' | ') : String(queries || '');
}

function getRetrievalCandidateFingerprint(item, query) {
  return `${getCandidateKey(item)}|${getSearchQueryText(query)}`;
}

function getCandidateScoreSummary(candidate) {
  return getCandidateScoreLabel(candidate);
}

function getSearchQueryMatchInfo(question, item) {
  return {
    keywordScore: getChunkKeywordScore(item.chunk, question),
    bm25Score: scoreIndexByBm25([item], [question])[0] || 0
  };
}

function getRetrievalMetrics(question, candidates) {
  return {
    query: question,
    retrieved: getCandidateCount(candidates),
    topScore: getBestScoreOnCandidates(candidates)
  };
}

function getCandidateMetaMatch(item, queryEntities) {
  return getCandidateMetadataScore(item);
}
...
function getHybridCandidateMetadata(item, queryEntities) {
  return getCandidateMetadataScore(item);
}

function getCandidateRetrievalMetadata(item) {
  return getChunkEntities(item);
}

function getSearchQueryFeatureSet(query) {
  return { terms: tokenizeForSearch(query) };
}

function getCandidateFeatureVector(item, question, qEmb, bm25Score) {
  return {
    semantic: scoreCandidateEmbedding(item, qEmb),
    bm25: bm25Score,
    keyword: getChunkKeywordScore(item.chunk, question),
...
function getCandidateProgramScore(item, queryEntities) {
  return getCandidateMetadataScore(item);
}

function getSearchQueryCandidateSummary(item, question) {
  return `${getCandidateSnippet(item)} [score:${getChunkKeywordScore(item.chunk, question).toFixed(2)}]`;
}

function getHybridRetrievalCandidateDebug(candidate) {
  return {
    id: getCandidateIdentifier(candidate.item),
...
function buildHybridRetrievalCandidateLabel(candidate) {
  return getCandidateTitle(candidate.item);
}

function getCandidateSourceTag(item) {
  return item && item.filename ? String(item.filename).split('/').pop() : 'chunk';
}

function getCandidateTopicFeatures(item, question) {
  return { overlap: getQueryTermCoverage(question, item) };
}

function getSearchQueryCandidateQuality(item, question, queryEntities) {
  return getCandidateMetadataScore(item) + getChunkKeywordScore(item.chunk, question);
}

function getCandidateEmbeddingSimilarity(itemA, itemB) {
  return contextSimilarity(itemA, itemB);
}

function scoreCandidateMatch(item, question) {
  return getChunkKeywordScore(item.chunk, question);
}

function getQueryExpansionDebug(queries) {
  return uniqueItems(queries);
}

function getCandidateRetrievalDebug(item, question, queryEntities) {
  return getSearchQueryMatchInfo(question, item);
}

function getSearchQueryVariantCoverage(query, item) {
  return scoreCandidateAgainstQuery(item, query);
}

function getHybridRetrievalCandidateQuality(candidate) {
  return candidate.compositeScore || 0;
}

function getSearchQueryAssociation(item, queryEntities) {
  return isLikelyStrongProgramMatch(item, queryEntities) ? 'program' : 'generic';
}

function getCandidateTextScore(item) {
  return String(item.chunk || '').length;
}

function getCandidateProgramHintScore(item, queryEntities) {
  return getCandidateMetadataScore(item);
}

function getCandidateSimilarityScore(itemA, itemB) {
  return contextSimilarity(itemA, itemB);
}

function getRetrievalCandidateQualityDetails(candidate) {
  return getHybridRetrievalCandidateDebug(candidate);
}

function getCandidateTrustScore(item) {
  return scoreSourceTrust(item) / 100;
}

function getCandidateScoreComponents(item, question, intent, queryEntities, bm25Score) {
  const semanticScore = scoreCandidateEmbedding(item, null);
  const breakdown = getChunkScoreBreakdown(item, question, intent, semanticScore, queryEntities);
  return { ...breakdown, bm25Score };
}

function getHybridRetrievalCandidateSummary(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateRetrievalPath(item) {
  return String(item.filename || item.trainingId || item.id || 'chunk');
}

function getCandidateInfo(item, question) {
  return {
    id: getCandidateIdentifier(item),
    score: getChunkKeywordScore(item.chunk, question)
  };
}

function getHybridRetrievalPath(question, queryEntities) {
  return buildQuerySearchPayload(question, buildSearchQueries(question, queryEntities));
}

function getQueryExpansionState(question, queryEntities) {
  return { queries: buildSearchQueries(question, queryEntities) };
}

function getCandidateQueryInfo(item, question, queryEntities) {
  return { programMatch: isLikelyStrongProgramMatch(item, queryEntities), keywords: getChunkKeywordScore(item.chunk, question) };
}

function getHybridRetrievalReport(question, queryEntities, candidates) {
  return { query: question, results: (candidates || []).slice(0, 3).map(getHybridRetrievalCandidateDebug) };
}

function getSearchQueriesForDebug(question, queryEntities) {
  return buildSearchQueries(question, queryEntities);
}

function getCandidateSubtext(item) {
  return String(item.chunk || '').slice(0, 80);
}

function getCandidateLength(item) {
  return String(item.chunk || '').split(/\s+/).length;
}

function getHybridCandidatesForQuestion(question, index, queryEntities, qEmb) {
  return getTopHybridCandidates(index, question, detectIntent(question), queryEntities, buildSearchQueries(question, queryEntities), qEmb);
}

function shouldUseQueryExpansionDebug() {
  return String(process.env.RAG_DEBUG_QUERY_EXPANSION || 'false').toLowerCase() === 'true';
}

function getCandidateRetrievalDebugLabel(item, queryEntities) {
  return getCandidateProgramHint(item) || 'generic';
}

function getHybridRetrievalCandidateFeatures(candidate) {
  return { semantic: candidate.semanticScore, bm25: candidate.bm25Score, keyword: candidate.keywordScore };
}

function getRetrievalCandidateToContext(candidate) {
  return candidate && candidate.item ? candidate.item : null;
}

function getQueryExpansionCandidateDebug(query) {
  return getSearchQueryFeatureSet(query);
}

function getHybridRetrievalSearchSignature(question, queryEntities) {
  return getQuerySearchFingerprint(buildSearchQueries(question, queryEntities));
}

function getCandidateRankingLabel(candidate) {
  return getCandidateScoreLabel(candidate);
}

function getCandidateSourceSummary(item) {
  return String(item.filename || item.trainingId || item.id || 'chunk');
}

function getHybridRetrievalDebugSummary(question, candidates) {
  return { question, top: (candidates || []).slice(0, 3).map(getHybridRetrievalCandidateDebug) };
}

function getCandidateMetadataSummary(item) {
  return getChunkEntities(item);
}

function getRetrievalCandidateSource(candidate) {
  return candidate && candidate.item ? getCandidateSourceTag(candidate.item) : null;
}

function getHybridRetrievalContext(question, queryEntities) {
  return buildSearchQueries(question, queryEntities).join(' | ');
}

function isCandidateMetadataRelevant(item, queryEntities) {
  return isLikelyStrongProgramMatch(item, queryEntities);
}

function getQueryEntityCoverage(item, queryEntities) {
  return isLikelyStrongProgramMatch(item, queryEntities) ? 1 : 0;
}

function getHybridRetrievalQueryMetrics(question, queryEntities, candidates) {
  return { queryCount: getQueryVariantCount(buildSearchQueries(question, queryEntities)), topScore: getBestScoreOnCandidates(candidates) };
}

function getCandidateSourceConfidence(item) {
  return scoreSourceTrust(item) / 100;
}

function getHybridRetrievalDetails(question, candidates) {
  return { query: question, candidateCount: getCandidateCount(candidates) };
}

function getSearchQueriesForMetrics(question, queryEntities) {
  return buildSearchQueries(question, queryEntities);
}

function getCandidateMatchMetadata(item, question) {
  return { keyword: getChunkKeywordScore(item.chunk, question) };
}

function getHybridRetrievalQualityReport(question, queryEntities, candidates) {
  return buildHybridRetrievalDebug(question, queryEntities, candidates);
}

function getCandidateTokenOverlap(itemA, itemB) {
  return contextSimilarity(itemA, itemB);
}

function getSearchQueryCandidateRank(item, question) {
  return getCandidateRankingLabel({ item, compositeScore: getChunkKeywordScore(item.chunk, question) });
}

function getHybridRetrievalCandidateSourceInfo(candidate) {
  return getCandidateSourceSummary(candidate.item);
}

function getCandidateRetrievalCoverage(item, question) {
  return getQueryTermCoverage(question, item);
}

function getHybridRetrievalStabilityLabel(candidates) {
  return Array.isArray(candidates) ? `stable:${candidates.length}` : 'stable:0';
}

function getCandidateTopicMatch(item, question) {
  return getCandidateWordOverlap(item, question);
}

function getHybridRetrievalBucket(item, question) {
  return isCandidateStrong(item, question) ? 'strong' : 'weak';
}

function getCandidateIntentScore(item, userIntent) {
  return getCandidateMetadataScore(item);
}

function getHybridRetrievalScope(question, queryEntities) {
  return buildSearchQueries(question, queryEntities);
}

function getCandidateScoreNormalization(candidate) {
  return normalizeSemanticConfidence(candidate.compositeScore);
}

function getHybridRetrievalCandidateIndex(candidate) {
  return candidate && candidate.item ? getCandidateIdentifier(candidate.item) : null;
}

function getCandidateContextPreview(item) {
  return getCandidateTextPreview(item);
}

function getHybridRetrievalCandidateLabel(candidate) {
  return getCandidateTitle(candidate.item);
}

function getCandidateMetadataHint(item) {
  return getChunkEntities(item);
}

function getHybridRetrievalCandidateSummaryText(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchMetrics(item, question) {
  return { overlap: getQueryTermCoverage(question, item), keywords: getChunkKeywordScore(item.chunk, question) };
}

function getHybridRetrievalCandidateSourceDetails(candidate) {
  return getCandidateSourceSummary(candidate.item);
}

function getCandidateSearchQuality(item, question) {
  return getChunkKeywordScore(item.chunk, question);
}

function getCandidateProgramAffinity(item, queryEntities) {
  return isLikelyStrongProgramMatch(item, queryEntities) ? 1 : 0;
}

function getHybridRetrievalSignalDetails(question, queryEntities) {
  return buildRetrievalQuerySummary(question, queryEntities);
}

function getCandidateQuestionMatch(item, question) {
  return getChunkKeywordScore(item.chunk, question);
}

function getHybridRetrievalStats(question, queryEntities, candidates) {
  return getRetrievalMetrics(question, candidates);
}

function getCandidateReference(item) {
  return getCandidatePath(item);
}

function getHybridRetrievalDebugInfo(question, queryEntities, candidates) {
  return buildHybridRetrievalDebug(question, queryEntities, candidates);
}

function getCandidateSemanticMatchScore(item, qEmb) {
  return scoreCandidateEmbedding(item, qEmb);
}

function buildHybridRetrievalCandidateDiagnostics(candidate) {
  return getRetrievalCandidateQualityDetails(candidate);
}

function getCandidateField(item, field) {
  return item && item[field] ? item[field] : null;
}

function getHybridRetrievalCandidateScoreSummary(candidate) {
  return getCandidateScoreLabel(candidate);
}

function getCandidateSearchSignal(item, question) {
  return getCandidateSearchMetrics(item, question);
}

function getHybridRetrievalCandidateContext(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateIntentMatch(item, userIntent) {
  return getCandidateIntentScore(item, userIntent);
}

function getHybridRetrievalCandidateDebugPayload(question, queryEntities, candidate) {
  return getHybridRetrievalCandidateDebug(candidate);
}

function getCandidateSearchHint(item, question) {
  return getCandidateMatchMetadata(item, question);
}

function getHybridRetrievalCandidateScoreInfo(candidate) {
  return getCandidateScoreLabel(candidate);
}

function getCandidateSearchWeight(item, question) {
  return getChunkKeywordScore(item.chunk, question);
}

function getHybridRetrievalCandidateQualityInfo(candidate) {
  return getHybridRetrievalCandidateDebug(candidate);
}

function getCandidateSearchCoverage(item, question) {
  return getQueryTermCoverage(question, item);
}

function getHybridRetrievalCandidateScoring(candidate) {
  return getCandidateScoreLabel(candidate);
}

function getCandidateSearchMatch(item, question) {
  return getSearchQueryMatchInfo(question, item);
}

function getHybridRetrievalCandidateSummaryLabel(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateQuality(item, question) {
  return getCandidateSearchQuality(item, question);
}

function getHybridRetrievalCandidateTrace(candidate) {
  return buildHybridRetrievalCandidateDebug(candidate);
}

function getCandidateSearchCandidateDebug(item, question) {
  return getSearchQueryMatchInfo(question, item);
}

function getHybridRetrievalCandidateDebugSummary(question, candidate) {
  return getHybridRetrievalCandidateDebug(candidate);
}

function getCandidateSearchVariantInfo(item, question) {
  return tokenizeForSearch(question);
}

function getHybridRetrievalCandidateSummaryInfo(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateFeatures(item, question) {
  return getCandidateSearchMetrics(item, question);
}

function getHybridRetrievalCandidateFeatureSet(candidate) {
  return getCandidateEmbeddingScore(candidate.item, null);
}

function getCandidateSearchCandidateSource(item, question) {
  return getCandidateSourceSummary(item);
}

function getHybridRetrievalCandidateSourceTrace(candidate) {
  return getCandidateSourceSummary(candidate.item);
}

function getCandidateSearchCandidateSourceQuality(item, question) {
  return getCandidateSourceSummary(item);
}

function getHybridRetrievalCandidateFeatureDetails(candidate) {
  return getHybridRetrievalCandidateDebug(candidate);
}

function getCandidateSearchCandidateSummary(item, question) {
  return getCandidateTextPreview(item);
}

function getHybridRetrievalCandidateStatus(candidate) {
  return getCandidateSourceSummary(candidate.item);
}

function getCandidateSearchCandidateContext(item, question) {
  return getCandidateTextPreview(item);
}

function getHybridRetrievalCandidateTopicLabel(candidate) {
  return getCandidateSourceSummary(candidate.item);
}

function getCandidateSearchCandidateMatchFeatures(item, question) {
  return getSearchQueryMatchInfo(question, item);
}

function getHybridRetrievalCandidateFeatureTrace(candidate) {
  return getCandidateSourceSummary(candidate.item);
}

function getCandidateSearchCandidateQualityTrace(item, question) {
  return getSearchQueryMatchInfo(question, item);
}

function getHybridRetrievalCandidateTraceInfo(candidate) {
  return getCandidateSourceSummary(candidate.item);
}

function getCandidateSearchCandidateSignal(item, question) {
  return getCandidateSearchMetrics(item, question);
}

function getHybridRetrievalCandidateSignal(candidate) {
  return getCandidateSourceSummary(candidate.item);
}

function getCandidateSearchCandidateSummaryTrace(item, question) {
  return getCandidateTextPreview(item);
}

function getHybridRetrievalCandidateSummaryTrace(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateDebugSummary(item, question) {
  return getCandidateTextPreview(item);
}

function getHybridRetrievalCandidateDebugTrace(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateSummaryInfo(item, question) {
  return getCandidateTextPreview(item);
}

function getHybridRetrievalCandidateSummaryTraceInfo(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateDebugInfo(item, question) {
  return getSearchQueryMatchInfo(question, item);
}

function getHybridRetrievalCandidateCandidateDebug(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateComplexity(item, question) {
  return getCandidateTextPreview(item);
}

function getHybridRetrievalCandidateSignalSummary(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRuntime(item, question) {
  return getCandidateTextPreview(item);
}

function getHybridRetrievalCandidateRuntime(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateLabelInfo(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalQualitySummary(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateDiagnostic(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateDiagnostic(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateDiagnosticInfo(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateDiagnosticSummary(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummary(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummary(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankTrace(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankTrace(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankDebug(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankDebug(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankInfo(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankInfo(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankDiagnostics(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankDiagnostics(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankMetrics(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankMetrics(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankData(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankData(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankOutput(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankOutput(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankResult(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankResult(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryInfo(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryInfo(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryDebug(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryDebug(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryMetrics(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryMetrics(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryData(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryData(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryOutput(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryOutput(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryResult(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryResult(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryTrace(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryTrace(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryInfoTrace(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryInfoTrace(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryDiagnostics(item, question) {
  return getCandidateTextPreview(item);
}

// Tracing helpers (single unique definitions)
function getCandidateHybridRetrievalCandidateRankSummaryDiagnostics(candidate) { return getCandidateTextPreview(candidate.item); }
function getCandidateSearchCandidateRankSummaryMetricsTrace(item, question) { return getCandidateTextPreview(item); }
function getCandidateHybridRetrievalCandidateRankSummaryMetricsTrace(candidate) { return getCandidateTextPreview(candidate.item); }
function getCandidateSearchCandidateRankSummaryDataTrace(item, question) { return getCandidateTextPreview(item); }
function getCandidateHybridRetrievalCandidateRankSummaryDataTrace(candidate) { return getCandidateTextPreview(candidate.item); }
function getCandidateSearchCandidateRankSummaryOutputTrace(item, question) { return getCandidateTextPreview(item); }
function getCandidateHybridRetrievalCandidateRankSummaryOutputTrace(candidate) { return getCandidateTextPreview(candidate.item); }
function getCandidateSearchCandidateRankSummaryResultTrace(item, question) { return getCandidateTextPreview(item); }
function getCandidateHybridRetrievalCandidateRankSummaryResultTrace(candidate) { return getCandidateTextPreview(candidate.item); }
function getCandidateSearchCandidateRankSummaryInfoTraceResult(item, question) { return getCandidateTextPreview(item); }
function getCandidateHybridRetrievalCandidateRankSummaryInfoTraceResult(candidate) { return getCandidateTextPreview(candidate.item); }
function getCandidateSearchCandidateRankSummaryDiagnosticsTrace(item, question) { return getCandidateTextPreview(item); }
function getCandidateHybridRetrievalCandidateRankSummaryDiagnosticsTrace(candidate) { return getCandidateTextPreview(candidate.item); }
function getCandidateSearchCandidateRankSummaryMetricsTraceResult(item, question) { return getCandidateTextPreview(item); }
function getCandidateHybridRetrievalCandidateRankSummaryMetricsTraceResult(candidate) { return getCandidateTextPreview(candidate.item); }
function getCandidateSearchCandidateRankSummaryDataTraceResult(item, question) { return getCandidateTextPreview(item); }
function getCandidateHybridRetrievalCandidateRankSummaryDataTraceResult(candidate) { return getCandidateTextPreview(candidate.item); }
function getCandidateSearchCandidateRankSummaryOutputTraceResult(item, question) { return getCandidateTextPreview(item); }
function getCandidateHybridRetrievalCandidateRankSummaryOutputTraceResult(candidate) { return getCandidateTextPreview(candidate.item); }
function getCandidateSearchCandidateRankSummaryInfoTraceResultDebug(item, question) { return getCandidateTextPreview(item); }
function getCandidateHybridRetrievalCandidateRankSummaryInfoTraceResultDebug(candidate) { return getCandidateTextPreview(candidate.item); }
function getCandidateSearchCandidateRankSummaryDiagnosticsTraceResult(item, question) { return getCandidateTextPreview(item); }
function getCandidateHybridRetrievalCandidateRankSummaryDiagnosticsTraceResult(candidate) { return getCandidateTextPreview(candidate.item); }

function getCandidateSearchCandidateRankSummaryMetricsTraceResultDebug(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryMetricsTraceResultDebug(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryDataTraceResultDebug(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryDataTraceResultDebug(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryOutputTraceResultDebug(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryOutputTraceResultDebug(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryResultTraceDebug(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryResultTraceDebug(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryInfoTraceResultDebugLabel(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryInfoTraceResultDebugLabel(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryDiagnosticsTraceResultDebug(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryDiagnosticsTraceResultDebug(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryMetricsTraceResultDebugLabel(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryMetricsTraceResultDebugLabel(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryDataTraceResultDebugLabel(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryDataTraceResultDebugLabel(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryOutputTraceResultDebugLabel(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryOutputTraceResultDebugLabel(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryResultTraceDebugLabel(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryResultTraceDebugLabel(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryInfoTraceResultDebugLabelAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryInfoTraceResultDebugLabelAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryDiagnosticsTraceResultDebugLabelAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryDiagnosticsTraceResultDebugLabelAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryMetricsTraceResultDebugLabelAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryMetricsTraceResultDebugLabelAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryDataTraceResultDebugLabelAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryDataTraceResultDebugLabelAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryOutputTraceResultDebugLabelAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryOutputTraceResultDebugLabelAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryResultTraceDebugLabelAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryResultTraceDebugLabelAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryInfoTraceResultDebugLabelAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryInfoTraceResultDebugLabelAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryDiagnosticsTraceResultDebugLabelAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryDiagnosticsTraceResultDebugLabelAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryMetricsTraceResultDebugLabelAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryMetricsTraceResultDebugLabelAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryDataTraceResultDebugLabelAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryDataTraceResultDebugLabelAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryOutputTraceResultDebugLabelAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryOutputTraceResultDebugLabelAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryResultTraceDebugLabelAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryResultTraceDebugLabelAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryInfoTraceResultDebugLabelAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryInfoTraceResultDebugLabelAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryDiagnosticsTraceResultDebugLabelAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryDiagnosticsTraceResultDebugLabelAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryMetricsTraceResultDebugLabelAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryMetricsTraceResultDebugLabelAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryDataTraceResultDebugLabelAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryDataTraceResultDebugLabelAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryOutputTraceResultDebugLabelAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryOutputTraceResultDebugLabelAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryResultTraceDebugLabelAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryResultTraceDebugLabelAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryInfoTraceResultDebugLabelAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryInfoTraceResultDebugLabelAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryDiagnosticsTraceResultDebugLabelAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryDiagnosticsTraceResultDebugLabelAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryMetricsTraceResultDebugLabelAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryMetricsTraceResultDebugLabelAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryDataTraceResultDebugLabelAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryDataTraceResultDebugLabelAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryOutputTraceResultDebugLabelAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryOutputTraceResultDebugLabelAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryResultTraceDebugLabelAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryResultTraceDebugLabelAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryInfoTraceResultDebugLabelAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryInfoTraceResultDebugLabelAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryDiagnosticsTraceResultDebugLabelAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryDiagnosticsTraceResultDebugLabelAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryMetricsTraceResultDebugLabelAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryMetricsTraceResultDebugLabelAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryDataTraceResultDebugLabelAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryDataTraceResultDebugLabelAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryOutputTraceResultDebugLabelAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryOutputTraceResultDebugLabelAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryResultTraceDebugLabelAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryResultTraceDebugLabelAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryInfoTraceResultDebugLabelAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryInfoTraceResultDebugLabelAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryDiagnosticsTraceResultDebugLabelAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryDiagnosticsTraceResultDebugLabelAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryMetricsTraceResultDebugLabelAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryMetricsTraceResultDebugLabelAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryDataTraceResultDebugLabelAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryDataTraceResultDebugLabelAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryOutputTraceResultDebugLabelAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryOutputTraceResultDebugLabelAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryResultTraceDebugLabelAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryResultTraceDebugLabelAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryInfoTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryInfoTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryDiagnosticsTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryDiagnosticsTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryMetricsTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryMetricsTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryDataTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryDataTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryOutputTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryOutputTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryResultTraceDebugLabelAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryResultTraceDebugLabelAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryInfoTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryInfoTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryDiagnosticsTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryDiagnosticsTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryMetricsTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryMetricsTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryDataTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryDataTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryOutputTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryOutputTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryResultTraceDebugLabelAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryResultTraceDebugLabelAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryInfoTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryInfoTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryDiagnosticsTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryDiagnosticsTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryMetricsTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryMetricsTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryDataTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryDataTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryOutputTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryOutputTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryResultTraceDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryResultTraceDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryInfoTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryInfoTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryDiagnosticsTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryDiagnosticsTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryMetricsTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryMetricsTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryDataTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryDataTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryOutputTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryOutputTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryResultTraceDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryResultTraceDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryInfoTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryInfoTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryDiagnosticsTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryDiagnosticsTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryMetricsTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryMetricsTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryDataTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryDataTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryOutputTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryOutputTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryResultTraceDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryResultTraceDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryInfoTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryInfoTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryDiagnosticsTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryDiagnosticsTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryMetricsTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryMetricsTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryDataTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryDataTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryOutputTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryOutputTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryResultTraceDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryResultTraceDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryInfoTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryInfoTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryDiagnosticsTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryDiagnosticsTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryMetricsTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryMetricsTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryDataTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryDataTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryOutputTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryOutputTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryResultTraceDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryResultTraceDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryInfoTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryInfoTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryDiagnosticsTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryDiagnosticsTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryMetricsTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryMetricsTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryDataTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryDataTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryOutputTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryOutputTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryResultTraceDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryResultTraceDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryInfoTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryInfoTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryDiagnosticsTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryDiagnosticsTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryMetricsTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryMetricsTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryDataTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryDataTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryOutputTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryOutputTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryResultTraceDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryResultTraceDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryInfoTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryInfoTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryDiagnosticsTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryDiagnosticsTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryMetricsTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryMetricsTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryDataTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryDataTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryOutputTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryOutputTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryResultTraceDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryResultTraceDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryInfoTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryInfoTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryDiagnosticsTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryDiagnosticsTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryMetricsTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryMetricsTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryDataTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryDataTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryOutputTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryOutputTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryResultTraceDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryResultTraceDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryInfoTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryInfoTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryDiagnosticsTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryDiagnosticsTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryMetricsTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryMetricsTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryDataTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryDataTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryOutputTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryOutputTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryResultTraceDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryResultTraceDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryInfoTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryInfoTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryDiagnosticsTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryDiagnosticsTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryMetricsTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryMetricsTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryDataTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryDataTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryOutputTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryOutputTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryResultTraceDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryResultTraceDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryInfoTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryInfoTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryDiagnosticsTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryDiagnosticsTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryMetricsTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryMetricsTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryDataTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryDataTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryOutputTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryOutputTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryResultTraceDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryResultTraceDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryInfoTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryInfoTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryDiagnosticsTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryDiagnosticsTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryMetricsTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryMetricsTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryDataTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryDataTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryOutputTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryOutputTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryResultTraceDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryResultTraceDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryInfoTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryInfoTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryDiagnosticsTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryDiagnosticsTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryMetricsTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryMetricsTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryDataTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryDataTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryOutputTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryOutputTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryResultTraceDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryResultTraceDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}



function getCandidateHybridRetrievalCandidateRankSummaryInfoTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryDiagnosticsTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryDiagnosticsTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryMetricsTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryMetricsTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryDataTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryDataTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryOutputTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryOutputTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryResultTraceDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryResultTraceDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}



function getCandidateSearchCandidateRankSummaryInfoTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateSearchCandidateRankSummaryInfoTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryInfoTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryDiagnosticsTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryDiagnosticsTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryMetricsTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryMetricsTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryDataTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryDataTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryOutputTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryOutputTraceResultDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function getCandidateSearchCandidateRankSummaryResultTraceDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(item, question) {
  return getCandidateTextPreview(item);
}

function getCandidateHybridRetrievalCandidateRankSummaryResultTraceDebugLabelAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgainAgain(candidate) {
  return getCandidateTextPreview(candidate.item);
}

function buildProgramAliasCandidate(raw) {
...
function deriveProgramAlias(raw) {
  return deriveProgramAliasFromModule(raw);
}
...
function normalizeProgramLabel(raw) {
  ...
  // Delegate acronym validation to deriveProgramAlias (stricter rules)
  const aliasFromDerive = deriveProgramAlias(value);
  if (aliasFromDerive && VALID_PROGRAMS.has(aliasFromDerive)) return aliasFromDerive;
