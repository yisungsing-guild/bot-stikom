...
function getAllowedAcademicCategories(intent) {
  switch (String(intent || '').toUpperCase()) {
    case 'DEFINISI_PRODI': return new Set(['PROGRAM_STUDI', 'INFO']);
...
function validateParsedFeeStruct(feeStruct, chunk) {
  if (!feeStruct || !chunk) return false;
  const numericFields = ['registrationFee', 'dpp', 'dppDiscount', 'registrationDiscount', 'ukt', 'scholarship'];
  for (const field of numericFields) {
    if (feeStruct[field]) {
      const validation = validateNumericGrounding(feeStruct[field], [{ chunk, filename: 'parsed' }]);
...
function parseFeeStructureFromChunk(item, queryEntities) {
  ...
  if (!validateParsedFeeStruct(feeStruct, chunk)) return null;
  return feeStruct;
}
...
function parseFeeStructure(chunks, queryEntities) {
  ...
  if (!queryEntities || !queryEntities.academicYear) {
    const bestYear = Math.max(...baseCandidates.map(c => parseYearKey(c.academicYear)));
    if (bestYear > 0) {
...
function tryStructuredExactCostAnswer(question, queryEntities, indexForQuery, topK, qEmb) {
  if (!queryEntities || queryEntities.intent !== 'COST') return null;
  ...
  try {
    console.log('[TRACE_COST_RAG_MATCH]', {
      question,
      queryEntities,
...
async function query(question, topK = 8, options = null) {
  try {
    ...
    const confidenceScoreTop = (top && top[0] && typeof top[0].score === 'number') ? top[0].score : topScoreAll;
