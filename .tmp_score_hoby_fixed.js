...
function tokenizeForRelevanceGuard(text) {
  const t = normalizeIndonesianQuestionText(text).replace(/[^\p{L}\p{N}\s]/gu, ' ').replace(/\s{2,}/g, ' ').trim();
  if (!t) return [];
...
function scoreProgramsFromHobbyLines(hobbyText, questionText) {
  ...
  for (const lineRaw of lines) {
    ...
    if (hit <= 0) continue;
    const cov = hit / Math.max(5, qTokens.length);
  ...
  return ranked.map(r=>({key:r.p.key,label:r.p.label,score:r.score,ev:r.ev}));
}
