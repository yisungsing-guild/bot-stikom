...
(async () => {
  try {
    ...
    for (const item of index) {
      ...
      scored.push({ id: item.id, filename: item.filename || item.sourceFile || null, program: item.program || (item.metadata && item.metadata.program) || null, docCategory: item.docCategory || item.category || null, semanticScore: sem, compositeScore: breakdown.compositeScore, finalScore: breakdown.finalScore, scoreComponents: breakdown, chunk: (item.chunk || '').trim().slice(0, 600), fullChunk: item.chunk || '' });
    }
    ...
    console.log(JSON.stringify({ query: q, intent, queryEntities, top: evidenceChecks }, null, 2));
  } catch (e) {
