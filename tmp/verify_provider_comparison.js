...
(async () => {
  const query = 'mana yang lebih mahal s1 sistem informasi atau s1 bisnis digital?';
  const res = await request(app).post('/provider/webhook').send({ chatId: 'verify-user-compare', text: query });
  const output = { query, status: res.status, body: res.body, sent };
  require('fs').writeFileSync('tmp/verify_provider_comparison_result.json', JSON.stringify(output, null, 2), 'utf8');
