...
const matchesSI = matches.filter(item => item && ((item.program && String(item.program).toUpperCase() === 'SI') || /\bSISTEM INFORMASI\b/i.test(String(item.chunk || '')) || /\bSI\b/.test(String(item.chunk || ''))));
console.log('matchesSI', matchesSI.length);
for (const item of matchesSI.slice(0, 20)) {
  console.log('***');
  console.log('id', item.id);
  console.log('filename', item.filename);
